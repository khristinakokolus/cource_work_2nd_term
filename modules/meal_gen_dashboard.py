...
external_stylesheets = ['https://codepen.io/chriddyp/pen/bWLwgP.css', 'url(assets/stylesheet.css)']
...
app.layout = html.Div([
    ...
    html.Div([
        html.H1(children="MEAL GENERATOR WITH ANALYSIS",
                style={'textAlign': 'center', "color": "white", "font-family": 'Bitter',
                       "font-size": "50px", "font-weight": "normal", "line-height": "54px", "margin": "0 0 54px"}),
        ], ),
    html.Div([
    ...
    html.Div([
        dcc.Input(id='calories-id', placeholder='Calories...', type='text', debounce=True,
                  style={"border": "2px solid #aaa", "border-radius": "4px",
                         "margin": "8px 0", "outline": "none", "padding": "8px", "box-sizing": "border-box"}),
    ...
    html.Div([
        dcc.Dropdown(
            ...
            style={'width': '220px', 'font-size': '90%', 'height': '40px', "align-items": "center",
    ...
    html.Div([
        dcc.Input(id='exclude-id', type='text', placeholder='Ingredients to exclude...', debounce=True,
                  style={"border": "2px solid #aaa", "border-radius": "4px",
                         "margin": "8px 0", "outline": "none", "padding": "8px", "box-sizing": "border-box"}),
        ], style={"width": "100%", "display": "flex", "align-items": "center", "justify-content": "center",
                  'marginBottom': '1em'}
    ...
    html.Div([
        html.Button('Click here to see the results and wait a bit', id='show-secret', className="btn"),
    html.Div(id='body-div')
        ], className="button"),
    ], style={'background-image': 'linear-gradient(rgba(0,0,0,0.85), rgba(0,0,0,0.85)), url("assets/food_web_app.jpg")',
              "background-size": "cover", "background-position": "center", 'bottom': '0', 'right': '0', 'left': '0',
           'top': '0'}),
...
def update_meal_generating(calories, diet, exclude, click, tab):
    if click is None:
        ...
        data = modules.main_functions.get_data_from_url(modules.main_functions.BASE_URL,
                                                        modules.default_information.DEFAULT_MEAL_PLANNER_URL,
                                                        option, more_options)
        ...
        recipes_meal_gen = RecipeSearch(modules.default_information.DEFAULT_MEAL_PLANNER_URL, data)
        ...
        output.append(html.H1("INGREDIENTS YOU NEED TO COOK THE DISHES",
                              style={'textAlign': 'center', "color": "#B22222", "font-family": 'Bitter',
                                     "font-weight": "normal", "font-size": "50px"}))
        ingredients = recipe_advanced_data.ingredients()
        output.append(dcc.Graph(id='main-info-graph', figure=modules.main_functions.ingredients_table(recipes,
                                                                                                      ingredients)))
        output.append(html.H1("ANALYSIS OF THE NUTRITION OF THE DISHES",
                              style={'textAlign': 'center', "color": "#B22222",
                                     "font-family": 'Bitter', "font-size": "50px", "font-weight": "normal",
                                                                       "line-height": "54px", "margin": "0 0 54px"}))
        output.append(dcc.Graph(id='main-info-graph', figure=pie_chart_mealgenerator(recipes_nutrition)))
        for i in range(len(recipes)):
            ...
            output.append(dcc.Graph(id='main-graph', figure=modules.main_functions.
                                    plot_nutrition(main_substances_analysis, "Main nutrients")))

            output.append(dcc.Graph(id='other-graph', figure=modules.main_functions.
                                    plot_nutrition(other_substances_analysis, "Vitamins, minerals, etc.")))
        res_urls = recipe_advanced_data.get_recipe_urls()
        output.extend([html.H1("HERE ARE RECIPE'S LINKS:",
                               style={"color": "#B22222", "font-family": 'Bitter', "font-weight": "normal"}),
